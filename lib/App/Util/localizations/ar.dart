const Map<String, String> ar = {
  "Username": "اسم المستخدم %s",
  'Login Screen': "شاشة تسجيل الدخول",
  'Write Username': "أدخل اسم المستخدم",
  'Password': "كلمة المرور",
  'Write Password': "أدخل كلمة المرور",
  'Login': "تسجيل الدخول",
  'Please fill in the blank fields.': "يرجى ملئ الحقول الفارغة.",
  'Failed to login. Invalid response.':
      "فشل في تسجيل الدخول. استجابة غير صالحة.",
  'Oops! It seems there was an issue with your request. Please try again.':
      "عذرًا! يبدو أن هناك مشكلة في طلبك. يرجى المحاولة مرة أخرى.",
  'Access Denied! You don\'t have permission to view this content.':
      "تم رفض الوصول! ليس لديك إذن لعرض هذا المحتوى.",
  'Sorry, we couldn\'t find what you were looking for. Please try again later.':
      "عذرًا، لم نتمكن من العثور على ما كنت تبحث عنه. يرجى المحاولة مرة أخرى لاحقًا.",
  'Something went wrong. Please try again or contact support if the issue persists.':
      "حدث خطأ ما. يرجى المحاولة مرة أخرى أو الاتصال بالدعم إذا استمر المشكلة.",
  'username is invalid': "اسم المستخدم غير صالح",
  'the password is wrong': "كلمة المرور خاطئة",
  'Please check your internet connection.': "يرجى التحقق من اتصالك بالإنترنت.",
  'No internet connection': "لا يوجد اتصال بالإنترنت.",
  'Cancel': "إلغاء",
  'Connection problem': "مشكلة في الاتصال",
  'Verifying...': "جاري التحقق...",
};
